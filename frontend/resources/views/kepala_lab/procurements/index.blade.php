<x-layout bodyClass="g-sidenav-show bg-gray-200">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
        <x-navbars.navs.auth titlePage="Draf Pengadaan"></x-navbars.navs.auth>
                    ...
                    <div class="card my-4">
                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                            <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center px-3">
                                <h6 class="text-white text-capitalize ps-3 mb-0">Draf Pengadaan Barang</h6>
                                <a href="{{ route('kepala-lab.procurements.create') }}" class="btn btn-sm btn-white mb-0">
                                    <i class="material-icons text-sm me-1">add</i> Buat Draf Baru
                                </a>
                            </div>
                        </div>
                                ...
                                <table class="table align-items-center mb-0">
                                    ...
                                    <tbody id="draftsTableBody">
                                        ...
                                        <tr class="draft-item" 
                                            ...
                                            data-date="{{ isset($draft['createdAt']) ? \Carbon\Carbon::parse($draft['createdAt'])->format('Y-m-d') : '2000-01-01' }}">
                                            <td>
                                                <div class="d-flex px-2 py-1">
                                                    <div class="icon icon-sm icon-shape bg-gradient-info shadow text-center border-radius-md me-2 d-flex align-items-center justify-content-center">
                                                        <i class="material-icons opacity-10 text-white" style="font-size: 16px;">description</i>
                                                    </div>
                                            ...
                                            <td class="align-middle text-center">
                                                <span class="text-secondary text-xs font-weight-bold">
                                                    {{ isset($draft['createdAt']) ? \Carbon\Carbon::parse($draft['createdAt'])->format('d M Y') : '-' }}
                                                </span>
                                            </td>
                                            <td class="align-middle">
                                                <a href="{{ route('kepala-lab.procurements.show', $draft['_id']) }}" class="text-info font-weight-bold text-xs me-2" title="Detail">
                                                    <i class="material-icons text-sm">visibility</i>
                                                </a>
                                                @if($draft['status'] === 'draft')
                                                <a href="{{ route('kepala-lab.procurements.edit', $draft['_id']) }}" class="text-warning font-weight-bold text-xs me-2" title="Edit">
                                                    <i class="material-icons text-sm">edit</i>
                                                </a>
                                                <form action="{{ route('kepala-lab.procurements.destroy', $draft['_id']) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus draf ini?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-link text-danger text-xs p-0 mb-0" title="Hapus">
                                        ...
                                        <tr>
                                            <td colspan="5" class="text-center py-4">
                                                <p class="text-secondary text-sm mb-0">Belum ada draf pengadaan.</p>
                                            </td>
                                        </tr>
                                        @endforelse
                                    </tbody>
